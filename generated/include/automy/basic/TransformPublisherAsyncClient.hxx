...
public:
	...
	uint64_t set_transform(std::shared_ptr<const ::automy::basic::Transform3D> new_transform, 
			const std::function<void()>& _callback = std::function<void()>(),
			const std::function<void(const std::exception&)>& _error_callback = std::function<void(const std::exception&)>());
...
private:
	...
	std::map<uint64_t, std::pair<std::function<void()>, std::function<void(const std::exception&)>>> vnx_queue_set_transform;
