
// AUTO GENERATED by vnxcppcodegen

#include <basic/package.hxx>
#include <basic/DataPacket.hxx>
#include <vnx/Input.h>
#include <vnx/Output.h>
#include <vnx/Visitor.h>
#include <vnx/Object.h>
#include <vnx/Struct.h>


namespace basic {


const vnx::Hash64 DataPacket::VNX_TYPE_HASH(0xa813899b1e9f7c5cull);
const vnx::Hash64 DataPacket::VNX_CODE_HASH(0x23376fc8eca55473ull);

vnx::Hash64 DataPacket::get_type_hash() const {
	return VNX_TYPE_HASH;
}

const char* DataPacket::get_type_name() const {
	return "basic.DataPacket";
}

std::shared_ptr<DataPacket> DataPacket::create() {
	return std::make_shared<DataPacket>();
}

std::shared_ptr<vnx::Value> DataPacket::clone() const {
	return std::make_shared<DataPacket>(*this);
}

void DataPacket::read(vnx::TypeInput& _in, const vnx::TypeCode* _type_code, const uint16_t* _code) {
	vnx::read(_in, *this, _type_code, _code);
}

void DataPacket::write(vnx::TypeOutput& _out, const vnx::TypeCode* _type_code, const uint16_t* _code) const {
	vnx::write(_out, *this, _type_code, _code);
}

void DataPacket::accept(vnx::Visitor& _visitor) const {
	const vnx::TypeCode* _type_code = get_type_code();
	_visitor.type_begin(*_type_code);
	_visitor.type_field(_type_code->fields[0], 0); vnx::accept(_visitor, time);
	_visitor.type_field(_type_code->fields[1], 1); vnx::accept(_visitor, payload);
	_visitor.type_end(*_type_code);
}

void DataPacket::write(std::ostream& _out) const {
	_out << "{";
	_out << "\"time\": "; vnx::write(_out, time);
	_out << ", \"payload\": "; vnx::write(_out, payload);
	_out << "}";
}

void DataPacket::read(std::istream& _in) {
	std::map<std::string, std::string> _object;
	vnx::read_object(_in, _object);
	for(const auto& _entry : _object) {
		if(_entry.first == "payload") {
			vnx::from_string(_entry.second, payload);
		} else if(_entry.first == "time") {
			vnx::from_string(_entry.second, time);
		}
	}
}

vnx::Object DataPacket::to_object() const {
	vnx::Object _object;
	_object["time"] = time;
	_object["payload"] = payload;
	return _object;
}

void DataPacket::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		if(_entry.first == "payload") {
			_entry.second.to(payload);
		} else if(_entry.first == "time") {
			_entry.second.to(time);
		}
	}
}

/// \private
std::ostream& operator<<(std::ostream& _out, const DataPacket& _value) {
	_value.write(_out);
	return _out;
}

/// \private
std::istream& operator>>(std::istream& _in, DataPacket& _value) {
	_value.read(_in);
	return _in;
}

const vnx::TypeCode* DataPacket::get_type_code() {
	const vnx::TypeCode* type_code = vnx::get_type_code(vnx::Hash64(0xa813899b1e9f7c5cull));
	if(!type_code) {
		type_code = vnx::register_type_code(create_type_code());
	}
	return type_code;
}

std::shared_ptr<vnx::TypeCode> DataPacket::create_type_code() {
	std::shared_ptr<vnx::TypeCode> type_code = std::make_shared<vnx::TypeCode>(true);
	type_code->name = "basic.DataPacket";
	type_code->type_hash = vnx::Hash64(0xa813899b1e9f7c5cull);
	type_code->code_hash = vnx::Hash64(0x23376fc8eca55473ull);
	type_code->is_class = true;
	type_code->create_value = []() -> std::shared_ptr<vnx::Value> { return std::make_shared<DataPacket>(); };
	type_code->fields.resize(2);
	{
		vnx::TypeField& field = type_code->fields[0];
		field.name = "time";
		field.code = {8};
	}
	{
		vnx::TypeField& field = type_code->fields[1];
		field.is_extended = true;
		field.name = "payload";
		field.code = {12, 1};
	}
	type_code->build();
	return type_code;
}


} // namespace basic


namespace vnx {

void read(TypeInput& in, ::basic::DataPacket& value, const TypeCode* type_code, const uint16_t* code) {
	if(!type_code) {
		throw std::logic_error("read(): type_code == 0");
	}
	if(code) {
		switch(code[0]) {
			case CODE_STRUCT: type_code = type_code->depends[code[1]]; break;
			case CODE_ALT_STRUCT: type_code = type_code->depends[vnx::flip_bytes(code[1])]; break;
			default: vnx::skip(in, type_code, code); return;
		}
	}
	const char* const _buf = in.read(type_code->total_field_size);
	{
		const vnx::TypeField* const _field = type_code->field_map[0];
		if(_field) {
			vnx::read_value(_buf + _field->offset, value.time, _field->code.data());
		}
	}
	for(const vnx::TypeField* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			case 1: vnx::read(in, value.payload, type_code, _field->code.data()); break;
			default: vnx::skip(in, type_code, _field->code.data());
		}
	}
}

void write(TypeOutput& out, const ::basic::DataPacket& value, const TypeCode* type_code, const uint16_t* code) {
	if(!type_code || (code && code[0] == CODE_ANY)) {
		type_code = vnx::write_type_code<::basic::DataPacket>(out);
		vnx::write_class_header<::basic::DataPacket>(out);
	}
	if(code && code[0] == CODE_STRUCT) {
		type_code = type_code->depends[code[1]];
	}
	char* const _buf = out.write(8);
	vnx::write_value(_buf + 0, value.time);
	vnx::write(out, value.payload, type_code, type_code->fields[1].code.data());
}

void read(std::istream& in, ::basic::DataPacket& value) {
	value.read(in);
}

void write(std::ostream& out, const ::basic::DataPacket& value) {
	value.write(out);
}

void accept(Visitor& visitor, const ::basic::DataPacket& value) {
	value.accept(visitor);
}

} // vnx
