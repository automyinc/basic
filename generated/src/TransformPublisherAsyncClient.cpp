...
uint64_t TransformPublisherAsyncClient::vnx_self_test(const std::function<void(const vnx::bool_t&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
int32_t TransformPublisherAsyncClient::vnx_purge_request(uint64_t _request_id, const vnx::exception& _ex) {
...
int32_t TransformPublisherAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	std::unique_lock<std::mutex> _lock(vnx_mutex);
	const auto _iter = vnx_pending.find(_request_id);
	if(_iter == vnx_pending.end()) {
		throw std::runtime_error("TransformPublisherAsyncClient: received unknown return");
	}
	const auto _index = _iter->second;
	vnx_pending.erase(_iter);
	switch(_index) {
		case 0: {
			const auto _iter = vnx_queue_vnx_get_config_object.find(_request_id);
			if(_iter == vnx_queue_vnx_get_config_object.end()) {
				throw std::runtime_error("TransformPublisherAsyncClient: callback not found");
			}
			...
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::vnx::ModuleInterface_vnx_get_config_object_return>(_value)) {
					...
					throw std::logic_error("TransformPublisherAsyncClient: invalid return value");
				}
			}
			break;
		}
		...
		default:
			if(_index >= 0) {
				throw std::logic_error("TransformPublisherAsyncClient: invalid callback index");
			}
	}
	return _index;
}
